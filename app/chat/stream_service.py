...
from app.monitoring.metrics import observe_llm_duration, observe_llm_ttft, record_llm_request, record_llm_tokens
from app.monitoring.trace_context import new_trace_id, reset_trace_id, set_trace_id
...
def get_mapping_value(value: object, *names: str) -> object | None:
    for name in names:
...
def coerce_token_count(value: object | None) -> int | None:
...
def extract_token_usage(value: object) -> tuple[int | None, int | None, int | None] | None:
    usage = get_mapping_value(value, "usage_metadata", "usageMetadata", "usage")
    if usage is None:
        return None
    prompt_tokens = coerce_token_count(get_mapping_value(usage, "prompt_token_count", "promptTokenCount", "prompt_tokens"))
    completion_tokens = coerce_token_count(
        get_mapping_value(usage, "candidates_token_count", "candidatesTokenCount", "completion_tokens")
    )
    total_tokens = coerce_token_count(get_mapping_value(usage, "total_token_count", "totalTokenCount", "total_tokens"))
...
async def call_suggested_questions_model(prompt: str, model_config: WebAssistantModelConfig) -> str:
    ...
    model_name = model_config.suggested_questions_model
    ...
    try:
        ...
        record_llm_request(operation, model_name, status)
        observe_llm_duration(operation, model_name, status, perf_counter() - started_at)
        if usage is not None:
            record_llm_tokens(operation, model_name, prompt_tokens=usage[0], completion_tokens=usage[1], total_tokens=usage[2])
...
async def stream_chat_response(
    ...
    try:
        ...
        with maybe_langwatch_trace(
            ...
            active_trace_id = get_langwatch_trace_id(langwatch_trace) or active_trace_id
            set_trace_id(active_trace_id)

            guardrail = await evaluate_input_guardrail(settings, request.message)
            if not guardrail.allowed:
                yield {"event": "error", "data": guardrail.message or GUARDRAIL_ERROR_MESSAGE}
                return
            ...
            user_message = Message(conversation_id=conversation.id, role="user", content=request.message, trace_id=active_trace_id)
            ...
            if has_reference_context(request) and request.context:
                ...
                reference_artifact = await save_page_text_artifact(
                    session=session,
                    user_id=user_id,
                    conversation_id=conversation.id,
                ...
                reference_artifact.trace_id = active_trace_id
                reference_artifacts.append(reference_artifact)
            ...
            for artifact in artifacts:
                ...
                artifact.trace_id = active_trace_id
            await session.commit()
            yield {
                "event": "conversation",
                "data": json.dumps(
            ...
            model_name = "mock" if settings.chat_agent_mode == "mock" else None
            started_at = perf_counter()
            status = "success"
            first_token_recorded = False
            latest_usage: tuple[int | None, int | None, int | None] | None = None
            try:
                if settings.chat_agent_mode == "mock":
                    final_response = build_mock_response(prompt)
                    observe_llm_ttft(operation, model_name, perf_counter() - started_at)
                    ...
                    async for event in runner.run_async(
                        ...
                        event_usage = extract_token_usage(event)
                ...
                if latest_usage is not None:
                    record_llm_tokens(operation, model_name, prompt_tokens=latest_usage[0], completion_tokens=latest_usage[1], total_tokens=latest_usage[2])
