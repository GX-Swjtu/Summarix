...
AI 对话通过 POST SSE 返回，前端用 fetch 解析 `text/event-stream`，实现动态打字机效果。模型通过 LiteLLM 接入，默认模型为 `qwen3.5-flash`，文本总结、视觉分析、通用对话、小红书文案和短视频脚本可以分别配置。
...
5. 用户打开文章页后，点击“小红书文案”或“短视频脚本”，插件提取网页正文并通过流式接口返回固定结构的改写结果。
...
- 插件可以将网页主体文章快捷转换为小红书文案和短视频脚本，并按固定模板输出。
- 云端历史可列出并查看详情。
- 模型偏好可读取和更新。
