...
> Run LangWatch evaluators inline on gateway traffic — pre, post, and per-stream-chunk.
...
* **Stream\_chunk modify** when real-time is essential. Example: the model is emitting a credit-card number mid-stream; redact before the user sees it.
...
* [Policy Rules](/ai-gateway/policy-rules) for simpler regex-based denials (tools, MCPs, URLs, models) that don't need an evaluator.
